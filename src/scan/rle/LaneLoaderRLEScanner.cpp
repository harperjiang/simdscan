...
#include "LaneLoaderRLEScanner.h"
...
LaneLoaderRLEScanner::LaneLoaderRLEScanner(uint32_t es, uint32_t rls) {
    this->entrySize = es;
    this->rlSize = rls;
    this->laneLoader = new LaneLoader64(es + rls);
    ...
    this->mask[0] = build(1 << (entrySize - 1), es + rls);
    this->notmask[0] = _mm512_xor_si512(this->mask[0], one);
}

LaneLoaderRLEScanner::~LaneLoaderRLEScanner() {
...
void LaneLoaderRLEScanner::scan(int *input, uint64_t length, int *output, Predicate *p) {
    ...
    this->val1[0] = build(p->getVal1(), entrySize + rlSize);
